...
#### 三重积分的概念
> * 物理意义：二重积分可以表示薄片的质量，那么相应的，三重积分的物理意义可以是表示空间物体的质量
> * 几何意义：我们在二重积分力讲过，二重积分可以表示曲顶面柱的体积，而我们所生活的是三维的世界，因此相应的对于我们所生活的世界，三重积分是没有几何意义的

**1. 定义**
设空间有界必区域$V$,$f(x,y,z)$为定义在$V$上的函数，用*三族曲面*将$V$分割成$n$个小立体$\Delta V_i$ ($i\in[1,n]$)，（$\Delta V_i$也表示第$i$个小立体的体积）。
在每一个小立体中任取一点$(\xi_i,\eta_i,\zeta_i)$作和$$S_n=\sum\limits_{i=1}^{n}f(\xi_i,\eta_i,\zeta_i)\Delta V_i$$
设$\lambda$为$\Delta V_i$地最大直径
> [!Note]
> $$\lim\limits_{\lambda\rightarrow 0}\sum\limits_{i=1}^{n} f(\xi_i,\eta_i,\zeta_i)\Delta V_i=\iiint\limits_V f(x,y,z)dV $$
#### 三重积分的计算
---
### 7.3重积分的应用
#### 几何应用——曲面面积
